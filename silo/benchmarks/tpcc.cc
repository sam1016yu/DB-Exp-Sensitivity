...
public:
  ...
  inline ~scoped_multilock()
  {
    ...
    if (verbose){
          cerr<<"Executing multilock destructor"<<endl;
    }

    if (did_lock)    //check this
    // if (verbose){
    //       cerr<<"DIDLOCK"<<endl;
    // }

      for (auto &t : locks){   //check loops
        if (verbose){
          cerr<<"Unlocking"+hexify(t)<<endl;
        }
        t->unlock();
        if (verbose){
          if (t->is_locked()){
            cerr<<"Unlock failed"+hexify(t)<<endl;
          }else{
            cerr<<"Unlocked"+hexify(t)<<endl;
            }
  ...
  {
    // if (verbose){
    //   cerr<<"start_multilock"<<endl;
    //   cerr<<"locks_size:"+std::to_string(locks.size())<<endl;
    // }
    ALWAYS_ASSERT(!did_lock);
...
// #ifdef CHECK_INVARIANTS
    if (set<T *>(locks.begin(), locks.end()).size() != locks.size()) {
      cerr<<"Duplicate locks found"<<endl;
      // for (auto &t : locks)
...
// #endif
    ...
    for (auto &t : locks){
          if (verbose){
            cerr<<"Attempting to lock"+hexify(t)<<endl;
          }
          t->lock();
          if (verbose){
          cerr<<"Locked"+hexify(t)<<endl;
        }
    }
...
LockForPartition(unsigned int wid)
{
  INVARIANT(g_enable_partition_locks);
    ...
    // if (verbose){
    //       cerr<<"WH:"+std::to_string(wid)+"==>"+hexify(g_partition_locks[wid].elem)<<endl;
    // }
...
public:
  ...
  get_workload() const
  {
    ...
    if (verbose){
    cerr<<"generated workload desc vector"<<worker_id<<endl;
  }
  }
...
protected:
  ...
  load()
  {
    ...
    if (verbose) {
      cerr << "[INFO] started loading warehouse" << endl;
    }
    string obj_buf;
    ...
    if (verbose) {
      ...
      cerr << "[INFO] Total WH tbl Size" << double(warehouse_total_sz)<< " bytes" << endl;
...
protected:
  ...
  load()
  {
    ...
    if (verbose) {
      cerr << "[INFO] started loading items" << endl;
    }
    string obj_buf;
    ...
    if (verbose) {
      ...
      cerr << "[INFO] Total Item Tbl Size" << double(total_sz)<< " bytes" << endl;
...
protected:
  ...
  load()
  {
    ...
    if (verbose) {
      cerr << "[INFO] started loading stock" << endl;
    }
    ...
    if (verbose) {
      if (warehouse_id == -1) {
        ...
        cerr << "[INFO] Total Stock Tbl Size (perWH)" << double(stock_total_sz)<< " bytes" << endl;
...
protected:
  ...
  load()
  {
    ...
    if (verbose) {
      cerr << "[INFO] started loading district" << endl;
    }
    string obj_buf;
    ...
    if (verbose) {
      ...
      cerr << "[INFO] Total District Tbl Size" << double(district_total_sz)<< " bytes" << endl;
...
protected:
  ...
  load()
  {
    string obj_buf;
      ...
      if (verbose) {
      cerr << "[INFO] started loading customer" << endl;
    }
    ...
    for (uint w = w_start; w <= w_end; w++) {
      if (verbose) {
        // cerr << "[customer] started loading customer@"+std::to_string(w) << endl;
      }
      if (pin_cpus)
        PinToWarehouseId(w);
      for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++) {
        if (verbose) {
          // cerr << "[customer] started loading district "+std::to_string(d) << endl;
        }
        for (uint batch = 0; batch < nbatches;) {
          if (verbose) {
            // cerr << "[customer] started loading batch "+std::to_string(batch) << endl;
          }
          scoped_str_arena s_arena(arena);
          void * const txn = db->new_txn(txn_flags, arena, txn_buf());
          ...
          try {
            for (uint cidx0 = cstart; cidx0 < cend; cidx0++) {
              ...
              total_sz += sz;
              // if (verbose) {
              //   cerr << "[customer] entry size@"+std::to_string(sz) << endl;
              // }
              tbl_customer(w)->insert(txn, Encode(k), Encode(obj_buf, v));
              // if (verbose) {
              //   cerr << "[customer] inserted into tbl_customer"<< endl;
              // }
              ...
              // if (verbose) {
              //   cerr << "[customer] inserted into tbl_customer_idx"<< endl;
              // }
              ...
              // if (verbose) {
              //   cerr << "[customer] inserted into tbl_history"<< endl;
              // }
            }
            if (db->commit_txn(txn)) {
              batch++;
    ...
    if (verbose) {
      if (warehouse_id == -1) {
        ...
        cerr << "[INFO] Total Customer Size (per WH)" << double(total_sz)<< " bytes" << endl;
...
protected:
  ...
  load()
  {
    ...
    if (verbose) {
      cerr << "[INFO] started loading orders" << endl;
    }
    string obj_buf;
    ...
    if (verbose) {
      if (warehouse_id == -1) {
        ...
        cerr << "[INFO] Total Order Tbl Size per WH" << double(order_line_total_sz)+double(oorder_total_sz)+double(new_order_total_sz)<< " bytes" << endl;
...
tpcc_worker::txn_new_order()
{
  ...
  const uint numItems = RandomNumber(r, 5, 15);
    ...
    // if (verbose){
    //   cerr<<" Beginning of new order "<<endl;
    // }
  ...
  if (g_enable_partition_locks) {
    ...
    mlock.multilock();
    // if (verbose){
    //   cerr<<"Warehouse Id: "<<warehouse_id<<" Lock Init Finished "<<endl;
    // }
  }
  try {
    // if (verbose){
    //       cerr<<"start_idgen"<<endl;
    // }
    ssize_t ret = 0;
...
#undef OPEN_TABLESPACE_X
    ...
    if (g_enable_partition_locks) {
      ...
      void * const px = memalign(CACHELINE_SIZE, sizeof(aligned_padded_elem<spinlock>) * NumWarehouses()+1);
      ALWAYS_ASSERT(px);
      ...
      for (size_t i = 0; i < NumWarehouses(); i++) {
        ...
        if (verbose){
              cerr<<"lock_allocated@"+hexify(&g_partition_locks[i].elem)<<endl;
        }
