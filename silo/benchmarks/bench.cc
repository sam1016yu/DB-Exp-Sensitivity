...
bench_worker::run()
{
  ...
  on_run_setup();
    if (verbose){
    cerr<<"worker setup finished@"<<worker_id<<endl;
  }
  scoped_db_thread_ctx ctx(db, false);
  if (verbose){
    cerr<<"context generated@"<<worker_id<<endl;
  }
  const workload_desc_vec workload = get_workload();
  if (verbose){
    cerr<<"workload finished loading@"<<worker_id<<endl;
  }
  ...
  while (running && (run_mode != RUNMODE_OPS || ntxn_commits < ops_per_worker)) {
    ...
    for (size_t i = 0; i < workload.size(); i++) {
        // if (benchverbose){
        //   cerr<<"BW_start_txn"<<endl;
        // }
      if ((i + 1) == workload.size() || d < workload[i].frequency) {
      retry:
        // if (benchverbose){
        //   cerr<<"BW_try_txn"<<endl;
        // }
        timer t;
        ...
        if (likely(ret.first)) {
          ...
          backoff_shifts >>= 1;
        //   if (benchverbose){
        //   cerr<<"BW_finish_txn"<<endl;
        // }
        } else {
          // cerr<<"BW_wating_restart"<<endl;
          ++ntxn_aborts;
