...
for (( i = 1; i <= $num_replica; i++ )); do
	echo "replica node${i}:$server_port" >> shard0.config
done

echo "f 1" > shard1.config
for (( i = 1 + $num_replica; i <= 2 * $num_replica; i++ )); do
	echo "replica node${i}:$server_port" >> shard1.config
done

echo "f 1" > shard2.config
for (( i = 1 + 2 * $num_replica; i <= 3 * $num_replica; i++ )); do
	echo "replica node${i}:$server_port" >> shard2.config
done

echo "f 1" > shard.tss.config
for (( i = 1 + 3 * $num_replica; i <= 3 * $num_replica + $nshard * $num_replica ; i++ )); do
	echo "replica node${i}:$server_port" >> shard.tss.config
done

for (( i = 0; i < 20; i++ )); do
	scp shard0.config node${i}:$HOME/tapir/store/tools
	scp shard1.config node${i}:$HOME/tapir/store/tools
	scp shard2.config node${i}:$HOME/tapir/store/tools
...
cd ../../ycsb-t
./run-tapir.sh $num_client_thd $nshard $3
