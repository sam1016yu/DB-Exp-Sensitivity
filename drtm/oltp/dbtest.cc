...
{
    // Tianxi: The below section is added to read env
    {
        // Read adaptive polling and durations
        g_adaptive_polling = stoi(read_env("ADPATIVE_POLLING", ADAPTIVE_POLLING_DEFAULT));
        g_pre_bpoll_duration = stoi(read_env("BPOLL_DURATION", PRE_BPOLL_DURATION_DEFAULT));
        g_epoll_timeout = stoi(read_env("EPOLL_TIMEOUT", EPOLL_TIMEOUT_DEFAULT));
        g_use_perf = stoi(read_env("USE_PERF", USE_PERF_DEFAULT));

        assert(g_pre_bpoll_duration > 0);
        assert(g_epoll_timeout > 0);

        LOG(INFO) << "Use adaptive polling=" << g_adaptive_polling;
    }

    extern const char* MakefileTimestampSignature;
    std::cout << "CFLGAS: " << MakefileTimestampSignature << std::endl;
    srand(time(NULL));

    memset(read_bytes, 0, MAX_THREADS * sizeof(uint64_t));
    memset(write_bytes, 0, MAX_THREADS * sizeof(uint64_t));
    memset(time_rdma_op, 0, MAX_THREADS * sizeof(uint64_t));
    
    memset(rdma_reads, 0, MAX_CLIENTS * MAX_THREADS * sizeof(uint64_t));
    memset(rdma_writes, 0, MAX_CLIENTS * MAX_THREADS * sizeof(uint64_t));
    memset(rdma_atomics, 0, MAX_CLIENTS * MAX_THREADS * sizeof(uint64_t));
    memset(rdma_searches, 0, MAX_CLIENTS * MAX_THREADS * sizeof(uint64_t));
  ...
  while (1) {
    ...
    int option_index = 0;
    int c = getopt_long(argc, argv, "b:s:t:d:B:f:r:n:o:m:l:a:p:c:x:C:D:E:F:G:", long_options, &option_index);
    if (c == -1)
      break;
    ...
    case 'C':
      g_rdma_size = std::strtoul(optarg, NULL, 10);
      break;
    case 'D':
      g_rdmatablesize = std::strtoul(optarg, NULL, 10);
      break;
    case 'E':
      g_records_per_thread = std::strtoul(optarg, NULL, 10);
      break;
    case 'F':
      g_reads_per_txn = std::strtoul(optarg, NULL, 10);
      break;
    case 'G':
      g_writes_per_txn = std::strtoul(optarg, NULL, 10);
      break;
...
//    cerr << "  num-cpus    : " << ncpus                     << endl;
    ...
    cerr << "  rdma-size: " << g_rdma_size << endl;
    cerr << "  rdmatablesize: " << g_rdmatablesize << endl;
    cerr << "  records-per-thread: " << g_records_per_thread << endl;
    cerr << "  reads-per-txn: " << g_reads_per_txn << endl;
    cerr << "  writes-per-txn: " << g_writes_per_txn << endl;
