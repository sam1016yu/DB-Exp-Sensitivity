...
int delivery_cnt=0;
int payment_cnt=0;
int order_status_cnt=0;
int stock_level_cnt=0;
int stock_level_rtm_cnt=0;
...
//	      fprintf(stdout,"full neworder: %f\n",neworder_transaction_thr);
	      ...
	      stringstream ss;
	      ss << endl << std::left
	        << setw(20) << "avg full thr=" << (full_transaction_thr / total_partition) << endl
	        << setw(20) << "agg full thr=" << full_transaction_thr << endl
            << setw(20) << "avg neworder thr=" << (neworder_transaction_thr / total_partition) << endl
            << setw(20) << "agg neworder thr=" << neworder_transaction_thr << endl;
#ifdef RATIO
...
//	      oss partition: %lld \n",total_cross_partition);
        ss
            << setw(20) << "cross wh=" << total_cross_warehouse << endl
            << setw(20) << "cross partion=" << total_cross_partition << endl;

#endif
	      LOG(INFO) << endl << dashed_line("Stats") << ss.str() << dashed_line() << endl;
	      ss.str("");
	    }
	    break;
	  }
      ...
      timestamp = mytimer::timestamp() >> 1;
#if 0
      int ret ;
      if ((ret = ntp_gettime(&tv)) == 0) {
    // fix DrTM's timestamp bug
	timestamp = (tv.time.tv_sec * 1000000000UL + tv.time.tv_usec * 1000UL) >> 1;
      } else {
	switch(ret) {
	case EFAULT:
...
//	   total_remote_ops,total_rdma_travel,total_rdma_read,total_remote_cas,total_local_cas);
    VLOG(2) <<
        "Total remote=" << total_remote_ops << ", travel=" << total_rdma_travel << ", read=" << total_rdma_read
        << ", r_cas=" << total_remote_cas << ", l_cas=" << total_local_cas << endl;
    delete node;
  }
...
//    fprintf(stdout,"worker id: %d\n",sstx.thread_id);
    ...
    if (verbose) {
      LOG(INFO) << "Create tpcc_worker, worker id " << worker_id
...
//    printf("Start ss %ld\n", store->ssman_->curSS);
    VLOG(2) << "Start snapshot, worker id=" << worker_id << ", current ss=" << store->ssman_->curSS;
    if (!pin_cpus)
      return;
  }
...
#if CHECK_FLAG
  VLOG(2) << "tpcc_worker, check_consistency";
...
tpcc_worker::txn_new_order()
{
  ...
  const uint warehouse_id = PickWarehouseId(r, warehouse_id_start, warehouse_id_end);
//  VLOG(1) << "txn_new_order, wh id=" << warehouse_id << ", total_cnt=" << ++pick_cnt << ", new_order=" << ++new_order_cnt;
...
{
  const uint warehouse_id = PickWarehouseId(r, warehouse_id_start, warehouse_id_end);
//    VLOG(1) << "txn_delivery, wh id=" << warehouse_id << ", total_cnt=" << ++pick_cnt << ", delivery_cnt=" << ++delivery_cnt;
...
//    VLOG(1) << "txn_payment, wh id=" << warehouse_id << ", total_cnt=" << ++pick_cnt << ", payment_cnt=" << ++payment_cnt;
...
{
  const uint warehouse_id = PickWarehouseId(r, warehouse_id_start, warehouse_id_end);
//    VLOG(1) << "txn_order_status, wh id=" << warehouse_id << ", total_cnt=" << ++pick_cnt << ", order_status_cnt=" << ++order_status_cnt;
...
tpcc_worker::txn_stock_level()
{
  ...
  const uint warehouse_id = PickWarehouseId(r, warehouse_id_start, warehouse_id_end);
//    VLOG(1) << "txn_stock_level, wh id=" << warehouse_id << ", total_cnt=" << ++pick_cnt << ", stock_level_cnt=" << ++stock_level_cnt;
...
// Tianxi: in regular tpcc test, txn_stock_level() is never called and the below is used.
tpcc_worker::txn_result
tpcc_worker::txn_stock_level_rtm()
{
  //this is the impl of read-only tx
  const uint warehouse_id = PickWarehouseId(r, warehouse_id_start, warehouse_id_end);
//    VLOG(1) << "txn_stock_level_rtm, wh id=" << warehouse_id << ", total_cnt=" << ++pick_cnt << ", stock_level_rtm_cnt=" << ++stock_level_rtm_cnt;
    const uint threshold = RandomNumber(r, 10, 20);
  const uint districtID = RandomNumber(r, 1, NumDistrictsPerWarehouse());
...
protected:
  ...
  bool check_consistency(){
	...
	continue;
      ...
      VLOG(2) << "tpcc_bench_runner, check_consistency";
  ...
  virtual void sync_log() {
    ...
    store->Sync();
    // Tianxi: The four counters below (NOCounter1,etc.) are not used.
...
//    // TODO: Double check on this one to see how Tinggang's can work on this without modifying
...
// Fix issues when scale_factor is greater than nthreads (still require sf to be multiples of nthreads)
...
tpcc_do_test( int argc, char **argv)
{
 ...
 abstract_db *db = NULL;
  ...
  if (verbose) {
    std::stringstream ss;
    ss << endl << "tpcc settings:" << endl
        << "\tcross_partition_transactions : " << !g_disable_xpartition_txn << endl
        << "\tread_only_snapshots          : " << !g_disable_read_only_scans << endl
        << "\tpartition_locks              : " << g_enable_partition_locks << endl
        << "\tseparate_tree_per_partition  : " << g_enable_separate_tree_per_partition << endl
        << "\tnew_order_remote_item_pct    : " << g_new_order_remote_item_pct << endl
        << "\tnew_order_fast_id_gen        : " << g_new_order_fast_id_gen << endl
        << "\tuniform_item_dist            : " << g_uniform_item_dist << endl
        << "\tworkload_mix                 : " <<
