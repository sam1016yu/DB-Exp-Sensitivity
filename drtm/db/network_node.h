...
//    fprintf(stdout,"tcp binding address %s\n",address);
  ...
  // This is used during handshake (msg="done" with pid and nid to notify remote to send back their RDMA ep info)
  // And after each node has finished work (msg=per_node_throughput),
  // the last worker of the node will send to each node (note that the node will also send the thru to itself),
  // but only the first node will aggregate the thru
...
//      fprintf(stdout,"mul estalabish %s\n",address);
        VLOG(2) << "MUL Send, address=" << address << ", msg=" << msg << ", pid=" << header[0] << ", worker_id=" << header[1];
#else
      if(_pid==0)
